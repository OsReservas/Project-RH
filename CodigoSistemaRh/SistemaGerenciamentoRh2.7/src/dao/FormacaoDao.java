...
public void atualizarFormacao(Formacao formacao) {
	
	String sql = "UPDATE formacao SET tipo = ?, nomeformacao = ?, instituicao = ?, inicio = ?, termino = ?, situacao =?" +
			"WHERE idformacao = ?";
	...
	try {
		...
		pstm.setString(5, formacao.getTerminoFormacao());
		pstm.setString(6, formacao.getSituacao());
		pstm.setInt(7, formacao.getId());
		pstm.execute();
		JOptionPane.showMessageDialog(null, "Formaçao atualizada com sucesso!");
	}catch (Exception ex) {
...
public void deletarFormacao(Formacao formacao) {
	...
	int opcao = JOptionPane.showConfirmDialog(null, "Tem certeza que deseja excluir a formação?", "Excluir", JOptionPane.YES_NO_OPTION);
